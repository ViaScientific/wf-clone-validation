...
process insert_qc {
    ...
    output:
        tuple val(meta), path("variant_files/${meta.alias}.insert.calls.bcf"), path("variant_files/${meta.alias}.insert.calls.bcf.csi"), path("variant_files/${meta.alias}.insert.calls.vcf"), path("${meta.alias}.insert.stats"), optional: true, emit: insert_stats
        tuple val(meta.alias), env(STATUS), emit: status
    ...
    if [ \$mapped != 0 ]; then
        ...
        mkdir -p variant_files
        bcftools mpileup -m1 -Ou -f insert_ref.fasta output.bam | bcftools call -mv -Ob -o "variant_files/${meta.alias}.insert.calls.bcf"
        bcftools index "variant_files/${meta.alias}.insert.calls.bcf"
        bcftools view -Ov "variant_files/${meta.alias}.insert.calls.bcf" > "variant_files/${meta.alias}.insert.calls.vcf"
        bcftools stats "variant_files/${meta.alias}.insert.calls.bcf" > "${meta.alias}.insert.stats"
        STATUS="Completed successfully"
    fi
    """
}
...
process assembly_comparison {
    ...
    output: 
        tuple val(meta), path("variant_files/${meta.alias}.full_construct.calls.bcf"), path("variant_files/${meta.alias}.full_construct.calls.bcf.csi"), path("variant_files/${meta.alias}.full_construct.calls.vcf"), path("${meta.alias}.full_construct.stats"), emit: full_assembly_stats
    script:
    ...
    mkdir -p variant_files
    bcftools mpileup -m1 -Ou -f full_reference.fasta "${meta.alias}.bam" | bcftools call -mv -Ob -P 1 -o "variant_files/${meta.alias}.full_construct.calls.bcf"
    bcftools index "variant_files/${meta.alias}.full_construct.calls.bcf"
    bcftools view -Ov "variant_files/${meta.alias}.full_construct.calls.bcf" > "variant_files/${meta.alias}.full_construct.calls.vcf"
    bcftools stats "variant_files/${meta.alias}.full_construct.calls.bcf" > ${meta.alias}.full_construct.stats
    """
}
...
process vcfReport {
    ...
    memory "2GB"
    input:
        tuple val(manifest), path(vcfs)
    output:
        path "variant_calls_summary.html", emit: html
    script:
        def manifestJson = new JsonBuilder(manifest).toPrettyString()
    """
    cat <<'EOF' > variants_manifest.json
    ${manifestJson}
EOF

    workflow-glue vcf_report --manifest variants_manifest.json --output variant_calls_summary.html
    """
}
...
process report {
    ...
    script:
        ...
        String metadata_str = metadata_obj.toPrettyString()
    ...
    """
    echo '${metadata_str}' > metadata.json
...
workflow pipeline {
    ...
    main:
        ...
        qc_insert = insert_qc_tuple.insert_stats.map{meta, bcf, idx, vcf, stats -> stats}
        bcf_insert = insert_qc_tuple.insert_stats.map{meta, bcf, idx, vcf, stats -> bcf}
        ...
        full_assembly_stats = assembly_comparison_output.full_assembly_stats.map{meta, bcf, idx, vcf, stats -> stats}
        bcf = assembly_comparison_output.full_assembly_stats.map{meta, bcf, idx, vcf, stats -> bcf}
        bam = qc_full.ref_aligned.map{meta, bam, bai, ref -> [meta, bam, bai]}

        variant_entries = insert_qc_tuple.insert_stats.map{meta, bcf, idx, vcf, stats -> [alias: meta.alias, category: 'insert', file: vcf] }
            .concat(assembly_comparison_output.full_assembly_stats.map{meta, bcf, idx, vcf, stats -> [alias: meta.alias, category: 'full_construct', file: vcf] })

        variant_manifest_channel = variant_entries
            .toList()
            .map { entries ->
                def manifest = entries.collect { [alias: it.alias, category: it.category, filename: it.file.getFileName().toString()] }
                def files = entries ? entries.collect { it.file } : [OPTIONAL_FILE]
                tuple(manifest, files)
            }

        variant_summary = vcfReport(variant_manifest_channel)
    ...
    emit:
        ...
        vcf_report = variant_summary // run-level channel
...
process publish {
    ...
    label "wfplasmid"
    publishDir "${params.outdir}", mode: 'copy', pattern: "*", saveAs: { filename -> filename }
    input:
        path fname
    output:
        path fname
...
// Publish variant files with directory structure preserved
process publishVariantFiles {
    label "wfplasmid"
    publishDir "${params.outdir}/variant_files", mode: 'copy', pattern: "*"
    input:
        path fname
    output:
        path fname
    """
    echo "Publishing variant files"
    """
}
...
workflow {
    ...
    | concat(
        ...
        results.insert_qc | map { meta, bcf, idx, vcf, stats -> stats } | flatten , 
        results.full_assembly_stats | map { meta, bcf, idx, vcf, stats -> stats } | flatten ,
    ...
    // Publish variant files with directory structure preserved
    results.insert_qc
    | map { meta, bcf, idx, vcf, stats -> [bcf, idx, vcf] }
    | flatten
    | concat(
        results.full_assembly_stats
        | map { meta, bcf, idx, vcf, stats -> [bcf, idx, vcf] }
        | flatten
    )
    | publishVariantFiles
